...
if (empty($_SESSION["user_session"])) {
	...
	if ($page == "airline" and $act == "list") {
		$primary = "t_airline.id";
		...
		while ($row = $query->fetch_row()) {
			...
			$data["3"] = $row["3"];
			$data["4"] = $row["4"];
			$data["5"] = "<button type='button' class='btn btn-mini btn-success' style='width:65px;' onclick=\"window.location.href='index.php?page=airline&act=detail&id=".$row["0"]."'\"><i class='icon-file icon-white'></i> DETAIL</button>";
		    $response["aaData"][] = $data;
		    $index++;
		}
		
		print($uri->json(json_encode($response)));
	} elseif ($page == "airline" and $act == "select") {
		$data = array();
		$query = $mysqli->query("select t_airline.id as id, t_airline.code as code, t_airline.name as name from t_airline where concat(t_airline.name, ' (', t_airline.code, ')') like '%".strtoupper($secure->sanitize($_GET["q"]))."%' and t_airline.active = 'Y' order by t_airline.id asc");
		
		while($r = $query->fetch_assoc()) {
			$data[] = array("id"=>$r["id"]."|".$r["code"], "text"=>$r["name"]." (".$r["code"].")");
		}
		
		print($uri->json(json_encode($data)));
	} elseif ($page == "airline" and $act == "insert") {
		...
		$query = $mysqli->query("select t_airline.id as id from t_airline where t_airline.code = '".$code."' and t_airline.name = '".$name."'");
		$row = $query->num_rows;
		
		if ($row <= 0) {
			$insert = $mysqli->query("insert into t_airline (code,
															 name,
			  								 			     active,
			                                 			  	 user,
														  	 modified) 
			...
			if (!$insert) {
				$_SESSION["airline"] = "ERROR OCCURED WHILE SUBMITTING DATA !";
				header("location:../../index.php?page=".$page."&act=new");
			} else {
				$id = $mysqli->insert_id;
				$_SESSION["airline"] = "AIRLINE <b>".$name." (".$code.")</b> HAS BEEN SUCCESSFULLY ADDED !";
				header("location:../../index.php?page=".$page."&act=detail&id=".$id);
			}
		} else {
			$_SESSION["airline"] = "AIRLINE <b>".$name." (".$code.")</b> IS ALREADY REGISTERED !";
			header("location:../../index.php?page=".$page."&act=new");
		}
	} elseif ($page == "airline" and $act == "update") {
		...
		if ($ori_code != $code or $ori_name != $name) {
			...
			if ($row <= 0) {
			  	$update = $mysqli->query("update t_airline set code = '".$code."',
			  												   name = '".$name."',
			  												   active = '".$active."',
													      	   user = '".$user."',
													      	   modified = '".$modified."'
				...
				if (!$update) {
					...
					$_SESSION["airline"] = "AIRLINE <b>".$name." (".$code.")</b> HAS BEEN SUCCESSFULLY UPDATED !";
					header("location:../../index.php?page=".$page."&act=detail&id=".$id);
				}
			} else {
				$_SESSION["airline"] = "AIRLINE <b>".$name." (".$code.")</b> IS ALREADY REGISTERED !";
