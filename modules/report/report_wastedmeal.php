...
$breadcrumb->append_crumb("WASTED MEAL SUMMARY", "#");
echo $breadcrumb->breadcrumb();

echo "<div class='panel'>
		<div class='panel-label'>WASTED MEAL REPORT</div>
	  	<div class='panel-page'>
	  		
		  	<div class='panel-info'>
			  	<div class='title pull-left'>WASTED MEAL SUMMARY</div>
			</div>
			...
			<div class='panel-action'>
			  	<div style='display:inline-block;height:30px;'></div>
					<div class='btn-group pull-left'>
					    ...
					    <ul class='dropdown-menu'>
					    	<li><a href='javascript:void(0)' onclick=\"view_report('".$action."?page=report&act=wastedmeal&avt=".$aviation."&flt=".$flight."&no=".$no."&from=".$from."&to=".$to."');\"><i class='icon-fullscreen icon-black'></i> VIEW</a></li>
							<li class='divider'></li>
							<li><a href='javascript:void(0)' onclick=\"window.location.href='".$action."?page=report&act=wastedmeal&avt=".$aviation."&flt=".$flight."&no=".$no."&from=".$from."&to=".$to."&export=word'\"><i class='icon-file icon-black'></i> WORD</a></li>
							<li class='divider'></li>
					    	<li><a href='javascript:void(0)' onclick=\"window.location.href='".$action."?page=report&act=wastedmeal&avt=".$aviation."&flt=".$flight."&no=".$no."&from=".$from."&to=".$to."&export=excel'\"><i class='icon-file icon-black'></i> EXCEL</a></li>
					    </ul>
				    </div>
					<form class='form-inline form-search pull-right' method='GET' action='".$_SERVER["PHP_SELF"]."'>
				  		<input type='hidden' name='page' value='report'>
						<input type='hidden' name='act' value='wastedmeal'>
				...
				<iframe id='report' class='report' src='".$action."?page=report&act=wastedmeal&avt=".$aviation."&flt=".$flight."&no=".$no."&from=".$from."&to=".$to."' frameborder='0'></iframe>
