...
if (empty($_SESSION["user_session"])) {
	...
	if ($page == "equipment" and $act == "list") {
		$primary = "t_equipment.id";
		
		$field = array (
			$primary,
		    "t_equipment.id",
		    "t_equipment.name",
		    "if(t_equipment.active = 'Y', 'YES', 'NO')"
		);
		...
		while ($row = $query->fetch_row()) {
			...
			$data["2"] = $row["2"];
			$data["3"] = $row["3"];
			$data["4"] = "<button type='button' class='btn btn-mini btn-success' style='width:65px;' onclick=\"window.location.href='index.php?page=equipment&act=detail&id=".$row["0"]."'\"><i class='icon-file icon-white'></i> DETAIL</button>";
		    $response["aaData"][] = $data;
		    $index++;
		}
		
		print($uri->json(json_encode($response)));
	} elseif ($page == "equipment" and $act == "select") {
		$data = array();
		$query = $mysqli->query("select t_equipment.id as id, t_equipment.name as name from t_equipment where t_equipment.name like '%".strtoupper($secure->sanitize($_GET["q"]))."%' and t_equipment.active = 'Y' order by t_equipment.id asc");
	...
	} elseif ($page == "equipment" and $act == "insert") {
		$name = strtoupper($secure->sanitize($_POST["name"]));
		$active = $secure->sanitize($_POST["active"]);
		$user = $_SESSION["user_id"];
		$modified = $datetime->server_datetime();
		
		$query = $mysqli->query("select t_equipment.id as id from t_equipment where t_equipment.name = '".$name."'");
		$row = $query->num_rows;
		
		if ($row <= 0) {
			$insert = $mysqli->query("insert into t_equipment (name,
			  								 			       active,
			                                 			  	   user,
														  	   modified) 
							                      	   values ('".$name."',
			...
			if (!$insert) {
				$_SESSION["equipment"] = "ERROR OCCURED WHILE SUBMITTING DATA !";
				header("location:../../index.php?page=".$page."&act=new");
			} else {
				$id = $mysqli->insert_id;
				$_SESSION["equipment"] = "EQUIPMENT <b>".$name."</b> HAS BEEN SUCCESSFULLY ADDED !";
				header("location:../../index.php?page=".$page."&act=detail&id=".$id);
			}
		} else {
			$_SESSION["equipment"] = "EQUIPMENT <b>".$name."</b> IS ALREADY REGISTERED !";
			header("location:../../index.php?page=".$page."&act=new");
		}
	} elseif ($page == "equipment" and $act == "update") {
		...
		if ($ori_name != $name) {
			...
			if ($row <= 0) {
			  	$update = $mysqli->query("update t_equipment set name = '".$name."',
			  													 active = '".$active."',
													      	     user = '".$user."',
													      	     modified = '".$modified."'
						                          	       where id = '".$id."'");
				...
				if (!$update) {
					...
					$_SESSION["equipment"] = "EQUIPMENT <b>".$name."</b> HAS BEEN SUCCESSFULLY UPDATED !";
					header("location:../../index.php?page=".$page."&act=detail&id=".$id);
				}
			} else {
				$_SESSION["equipment"] = "EQUIPMENT <b>".$name."</b> IS ALREADY REGISTERED !";
